/ You will build this in project part B - this is merely a
  ...
  void make_packet(Packet &p, ConnectionToStateMapping<TCPState> &CTSM, TYPE HeaderType, int size, bool timedOut){
          ...
          switch (HeaderType) {
                  case SYN: { SET_SYN(flags);
                    	      break;
                    	    } 
  

                  case ACK: { SET_ACK(flags);
                    	      break;
                    	    }
                  
                  case SYNACK: { SET_SYN(flags);
				 SET_ACK(flags);
                    	         break;
                    	       }
                  
                  case PSHACK: { SET_PSH(flags);
                     		 SET_ACK(flags);;
                    	         break;
                    	       }            
                  
                  case RESET: { SET_RST(flags);
                    	        break;
                    	      }  

		  case FIN: { SET_FIN(flags);
                    	      break;
                    	    }

		  case FINACK: { SET_FIN(flags);
                    		 SET_ACK(flags);
                    		 break;
                    	       }
